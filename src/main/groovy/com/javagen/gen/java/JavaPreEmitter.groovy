...
import static com.javagen.gen.model.MMethod.Stereotype.adder
...
import static com.javagen.gen.model.MMethod.Stereotype.getter
import static com.javagen.gen.model.MMethod.Stereotype.hash
import static com.javagen.gen.model.MMethod.Stereotype.setter
 ...
 */
class JavaPreEmitter extends MVisitor
{
    final EnumSet<MMethod.Stereotype> CLASS_METHODS = EnumSet.of(equals, hash, toString)
    EnumSet<MMethod.Stereotype> defaultMethods = EnumSet.of(equals, hash, toString, getter, setter, adder)
    ...
    private def toStringMethodBody(MMethod m, MVisitor v)
    {
    ...
    private def constructorMethodBody(MMethod m, MVisitor v)
    {
        if ( ! m.parent.isEnum() )
            v.out << '\n' << v.tabs << 'super();'
            ...
            switch (it) {
                ...
                case adder:
                    ...
                    if (!method) {
                        ...
                        adderName = 'add' + (singular ? upperCase(singular) : upCaseName)
    ...
    public static def getterMethodBody(MMethod m, MVisitor v)
    {
        MProperty prop = m.refs['property']
        def propName = prop.name
        v.out << '\n' << v.tabs << 'return ' << propName << ';'
    }
    public static def setterMethodBody(MMethod m, MVisitor v)
    {
        MProperty prop = m.refs['property']
        def propName = prop.name
        v.out << '\n' << v.tabs << 'this.' << propName << ' = ' << propName << ';'
    }
    public static def putterMethodBody(MMethod m, MVisitor v)
    {
    ...
    public static def adderMethodBody(MMethod m, MVisitor v)
    {
        MProperty prop = m.refs['property']
        def propName = prop.name
    ...
    private def equalsMethodBody(MMethod m, MVisitor v)
    {
//		@Override
...
//		}
    ...
    private def hashCodeMethodBody(MMethod m, MVisitor v)
    {
//		@Override
//		public int hashCode()
//		{
