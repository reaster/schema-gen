 ...
 * for navigating the tree using trait syntax: SchemaVisitor.super.visit(schema).
 */
trait SchemaVisitor {
    boolean printTrace = false

    def visit(Schema schema) {
        if (printTrace) println "schema @targetNamespace=${schema.prefixToNamespaceMap[Schema.targetNamespace]}"
    ...
    def visit(Union union) {
        print "union @name=${union.qname.name}"
